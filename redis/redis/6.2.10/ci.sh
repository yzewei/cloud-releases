...
# 继续后续的构建操作
cd /build/redis && \
#    make && mkdir redis-bin/ && cp src/redis-server redis-bin/ && cp src/redis-cli redis-bin/ && cp redis.conf redis-bin/ && tar zcvf redis-6.2.10.tar.gz redis-bin/ && \
    make && cd ../ && tar zcvf redis-6.2.10.tar.gz /build/redis && \
    bash /build/lib/curl.sh /build/redis-6.2.10.tar.gz redis/redis 6.2.10
