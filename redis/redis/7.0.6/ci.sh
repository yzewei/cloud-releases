...
# 继续后续的构建操作
cd /build/redis && \
#    make && mkdir redis-bin/ && cp src/redis-server redis-bin/ && cp src/redis-cli redis-bin/ && cp redis.conf redis-bin/ && tar zcvf redis-7.0.6.tar.gz redis-bin/ && \
    make && cd ../ && tar zcvf redis-7.0.6.tar.gz /build/redis && \
    bash /build/lib/curl.sh /build/redis-7.0.6.tar.gz redis/redis 7.0.6
